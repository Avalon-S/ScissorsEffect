#!/usr/bin/env python
"""
Generate Fig 2 left panel: Gradient Visualization with LGC annotations.
...
Output: figures/fig2_gradient_analysis.{png,jpg}
"""
import os
import sys
import argparse
...
def main():
    parser = argparse.ArgumentParser(description="Generate Fig 2 gradient visualization")
    ...
    parser.add_argument("--output_dir", type=str,
                        default=r"results/paper_fig2_gradient_vis")
    ...
    # Load data
    x_test, y_test = load_dataset(n_examples=args.n_examples)

    # Load models
    std_model = get_model("ResNet18_Standard", "cifar10", "Linf", device)
    std_model.eval()
    rob_model = get_model("Engstrom2019Robustness", "cifar10", "Linf", device)
    rob_model.eval()
    ...
    # Sort by distance from mean, pick the closest n_images
    ...
    # Representative LGC values for column headers
    rep_std_lgc = std_lgc_all[selected_indices[0]]
    rep_rob_lgc = rob_lgc_all[selected_indices[0]]

    # Column titles
    axes[0, 0].set_title("Original Image", fontsize=11, fontweight='bold')
    axes[0, 1].set_title(f"Standard (ResNet18)\nLGC: {std_mean:.2f}", fontsize=11, fontweight='bold')
    ...
    for ext in ['png', 'jpg']:
        out_path = os.path.join(args.output_dir, f"fig2_gradient_analysis.{ext}")
